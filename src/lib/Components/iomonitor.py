...
logging.basicConfig()
logger = logging.getLogger('iomon')
...
MAX_SYSTEM_IO_CAPACITY = 1024.0
MAX_PER_NODE_IO_CAPACITY = 1.0
...
class IOmonitor(Component):
    """IO Monitor. Monitor runtime I/O usage, logging into output log or provide to scheduler for I/O-aware job scheduling"""
    
    implementation = "iomon"
    name = "iomon"
    ...
    def init_iomon_logger(self):
        if self.iomon_logger == None:
            self.iomon_logger = PBSlogger(self.bqsim.get_outputlog_string() + "-iomon")
          
    def monitor_io(self):
        iobandwidth = self.bqsim.get_running_job_io_usage()
        msg = "%s;%s" % (self.get_current_time_date(), iobandwidth)
        self.iomon_logger.LogMessage(msg)
